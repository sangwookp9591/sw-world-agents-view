...
export default function SharedSessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  if (!session) {
    return (
      <div style={{ background: '#0a0a0f', color: '#4444ff', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace' }}>
        Loading session...
      </div>
    );
  }
  ...
  return (
    <div style={{ background: '#0a0a0f', color: '#e0e0f0', height: '100vh', fontFamily: 'monospace', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px 24px', borderBottom: '2px solid #1a1a2e', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <a href="/" style={{ marginLeft: session.sharing?.allowRemoteControl ? 0 : 'auto', color: '#4444ff', fontSize: 12, textDecoration: 'none' }}>
          ← Office
        </a>
      </div>
